...
const orderingOption = [
    ...
    { label: "Controversial", value: "interaction_diff_ordered" },
]

export default function SavedPosts({ userId }: { userId: number }) {
    const [ordering, setOrdering] = useState("-created_at");
    const { data: postsResponse, isLoading: isLoadingPosts, mutate: mutatePosts, size: postsSize, setSize: setPostsLimit, error } = useSWRInfinite((index) => ['/api/saved-posts', index, ordering, userId], ([, index]) => getUserSavedPosts(ordering, PAGE_SIZE, index + 1));
        ...
        <div>
            {
                ...
                <div className="my-5">
                    <h2 className="text-lg text-center font-semibold">No Saved Posts Yet.</h2>
                </div>
                :
                <div>
