...
export default function PendingPosts({ communityId }: { communityId: number }) {
    const [ordering, setOrdering] = useState("-created_at");
    const { data: postsResponse, isLoading: isLoadingPosts, mutate: mutatePosts, size: postsSize, setSize: setPostsLimit, error } = useSWRInfinite((index) => ['/api/posts', index, ordering, communityId, 'pending'], ([, index]) => getCommunityPosts(communityId, 'pending', ordering, PAGE_SIZE, index + 1));
        ...
        <div className="pt-4 mt-4 border-t-[1px] border-secondary">
            {
                ...
                <div className="my-5">
                    <h2 className="text-lg text-center font-semibold">No Pending Posts Yet.</h2>
                </div>
                :
                <div>
                        ...
                        <>
                            {
                                ...
                                <div className="my-5">
                                    <h2 className="text-lg text-center font-semibold">No Pending Posts.</h2>
                                </div>
                                :
                                posts?.map((post: Post) => (
                                    <div key={post.id}>
