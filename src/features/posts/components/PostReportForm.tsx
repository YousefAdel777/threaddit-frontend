...
import { createPostReportMutation, createPostReportOptions } from "@/lib/mutations";
...
export default function PostReportForm<T extends Post | Post[][]>({ postId, communityId, data: postsData, mutate, closeModal }: Props<T>) {
    ...
    const onSubmit = async (data: SchemaProps) => {
        setErrorMessage("");
        await mutate(
            createPostReportMutation(data, postsData),
            createPostReportOptions(data, postsData),
        );
        closeModal();
    }
